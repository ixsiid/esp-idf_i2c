...
I2CMaster::I2CMaster(const wire_s* wire, TickType_t wait) : I2CMaster(wire->i2cnum, wire->io_scl, wire->io_sda, wire->i2c_speed, wait) {}
I2CMaster::I2CMaster(i2c_port_t port, gpio_num_t scl, gpio_num_t sda, uint32_t freq_hz, TickType_t wait) {
	this->port = port;
	this->wait = wait;
...
esp_err_t I2CMaster::begin_transmission(uint8_t address, uint8_t registry) {
	...
	last_error = i2c_master_cmd_begin(port, cmd, wait);
